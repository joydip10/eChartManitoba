...
contract EChartBlockchain {
    ...
    event RecordAdded(uint256 healthID, string hl7Data, string recordType, uint256 providerID, uint256 sourceID, uint256 timestamp);
    ...
    function registerPatient(
        ...
        patients[newHealthID].healthID = newHealthID;
        patients[newHealthID].patientAddress = patientAddress;
        patients[newHealthID].dateOfBirth = dateOfBirth;
        patients[newHealthID].gender = gender;
        patients[newHealthID].exists = true;
    ...
    function addMedicalRecord(
        ...
        emit RecordAdded(healthID, hl7Data, recordType, providerID, sourceID, block.timestamp);
    }

    function grantSpecialAccess(uint256 healthID, address authorizedAddress) external onlyAdmin {
        require(patients[healthID].exists, "Patient not found");
    ...
    function revokeSpecialAccess(uint256 healthID, address authorizedAddress) external onlyAdmin {
        require(patients[healthID].exists, "Patient not found");
    ...
    function viewMedicalRecords(uint256 healthID) external returns (MedicalRecord[] memory) {
        ...
        accessLogs[healthID].push(AccessLog({
            ...
            timestamp: block.timestamp,
            providerID: providerAddressToID[msg.sender]
        }));
...
function getMedicalRecords(uint256 healthID) external view returns (MedicalRecord[] memory) {
    ...
    function getSystemOverview() external onlyAdmin view returns (
        ...
        totalRecords = medicalRecords[1].length;  
    }


function whoAmI() external view returns (
    string memory role,
    string memory idInfo,
    string memory extraInfo,
    uint256 count
) {
    
    if (registeredPatientAddress[msg.sender]) {
